    ...
    for (let i = 0; i < processedUsers.length; i += BATCH_SIZE) {
      ...
      await Promise.all(
        batch.map(async (user) => {
          try {
            // Skip if user already has a rating
            if (user.rating && user.ratedAt) {
              console.log(`Skipping ${user._id} - already rated`);
              return;
            }
            ...
            if (
              ...
              console.log(
                `[${userData.login}] Using existing web research data`
              );
              webResearchInfo = {
                openAI: {
